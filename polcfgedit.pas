...
uses
  ...
  Zipper, externprogsel, new_server, Process, httpsend, fphttpclient, ssl_openssl,
  WinDirs, ShellAPI, ShFolder{*, Types*};
...
type
  ...
  { TPOLConfigurator }
    ...
    CreateNewServerButton: TButton;
    Edit1: TEdit;
    GroupBox1: TGroupBox;
    DLStatusLabel: TLabel;
    ServerDLProgressLabel: TLabel;
    NewServerLabel: TLabel;
    ...
    procedure CreateNewServerButtonClick(Sender: TObject);
...
var
    ...
    ConfiguratorDatFile: TextFile;			        // A file written to the POL Installation with data relative to that installation such as World Data path.
    ...
    UOClientPath: String;							// String pointing to the uoclient.cfg file, including the "uoclient.cfg" name
    UOClientFile: TextFile;						    // File ref for uoclient.cfg
    EcompileFile: TextFile;						    // File ref for EcompileTab.cfg
    Configuratorbat: TextFile;					    // Used to create a temp batch file until I can learn to do this properly.
    ConfigIni: TIniFile;							// File ref for the ini file to hold info that needs to survive program stop/restart
    Instructions: TextFile;						    // File ref for the instructions for using the Configurator
    PolContents: Array[0..1000] of String;			// Buffer that contains the entire POLConfigurator.cfg file including comments.
    ...
    PolCfgArraySize: Integer;						// Holds the last position accessed in the array containing POLConfigurator.cfg during default cfg load
    ServSpecOptCfgArraySize: Integer;				// Holds the last position accessed in the array containing servspecopt.cfg during default cfg load
    CombatCfgArraySize: Integer;					// Holds the last position accessed in the array containing combat.cfg during default cfg load
    WatchCfgArraySize: Integer;						// Holds the last position accessed in the array containing watch.cfg during default cfg load
    ServersCfgArraySize: Integer;					// Holds the last position accessed in the array containing servers.cfg during default cfg load.
    ServerList: array[0..49] of ServersRec;			// Array of possible number of defined servers. 20 ought to be enough.
    ListenerList: array[0..49] of ListenerRec;		// Array of possible number of defined listeners. 20 ought to be enough.
    ServerCount: Integer;							// Counter of defined servers found in servers.cfg and stored in ServerList.
    ListenerCount: Integer;							// Counter of defined listeners found in uoclient.cfg and stored in ServerList.
    UOClientCfgArraySize: Integer;        			// Holds the last position accessed in the array containing uoclient.cfg during default cfg load.
    EcompileCfgArraySize: Integer;		        	// Holds the last position accessed in the array containing EcompileTab.cfg during default cfg load
    CoreHandledTags: String;						// Holds the hex string value read in and written for CoreHandledTags setting.
    CoreHandledTagsDec: Integer;					// Holds the decimal value of CoreHandledTags first 5 bits.
    I: Integer;										// Used to index through the cfg arrays while reading and writing the files
    ...
    StList: TStrings;								// Used as scratch storage when files are read in on Tstr parameters
    EqualsPos: Integer;								// Used to hold the position of the '=' sign in lines read from cfg files.
    Idex: Integer;									// Temp var used to store index data
    PadChars: TNoiseChar;							// Used to sanitize unwanted whitespace, etc. from lines read in from cfg files.
    ParamName: String;								// Used during reading of standard name/value pairs in POL cfg files.
    ParamVal: String;								//   "     "      "           "      "    "     "       "   "    "
    Delimiters: TSysCharSet;						// Used in the ParseLine() function.
    PolCfgFileParams : Array[1..67] of VarRec;		// Contains only the variables, their types and their values read in from POLConfigurator.cfg
    ...
    POLDirSet: Boolean = False;						// Flag gets set to True when a valid POL directory is entered.
    UODirSet: Boolean = False;						// Flag gets set to True when a valid UO installation directory is entered.
    MulsCopied: Boolean = False;					// Flag gets set when muls are copied from UO Dir to POL Dir
    UseSpecoptLocal: Boolean = False;				// If the user has a servspecopt.local.cfg file this will get set to True in the LoadCFGFiles procedure.
    ModuleDir: String;								// Used by read default EcompileTab to hold relative reference to module directory
    IncludeDir: String;								// Used by read default EcompileTab to hold relative reference to include directory
    PolScriptRootDir: String;						// Used by read default EcompileTab to hold relative reference to script directory
    PackageRootDir: String;							// Used by read default EcompileTab to hold relative reference to package directory
    PkgRootComboLen: Integer = 0;					// The number of items in the PkgRootCombo box.
    JunkArray : Array[0..1] of String;				// Used as scratch space for values returned by ParseLine.
    Profile : ProfRec;								// Record for profiles containing ProfName, PolLoc, UOLoc.
    ...
    DataPathsLoaded: Boolean = False;				// Indicates whether the POL installation has a Configurator.dat file with the Path Settings
													// and they were loaded.
    PathsSet: Boolean = False;						// Used to determine whether the paths on eCompile and Server Settings pages are set.
...
// Future functionality planned to make downloading the Core and Distro.
...
//  ShowMessage(GetWindowsSpecialDir(jint, True));
...
procedure TPOLConfigurator.CreateNewServerButtonClick(Sender: TObject);
...
begin
  ...
  Begin
    ServerPath := SelectDirectoryDialog1.FileName;
    ServerDLProgressLabel.Caption:='Downloading the POL Core';
    URL := 'https://github.com/polserver/ModernDistro/archive/master.zip';
    FileDownload(URL, True, 'Distro.zip');
    CopyFile(ExtractFilePath(Application.ExeName) + '/' + 'Distro.zip', ServerPath + '/' + 'Distro.zip');
    ServerDLProgressLabel.Caption:='Downloading the POL Core';
    ...
    CopyFile(ExtractFilePath(Application.ExeName) + '/' + 'pol.zip', ServerPath + '/' + 'pol.zip');
    ServerDLProgressLabel.Caption:='Unzipping th files.';
    // Do the unzipping magic.

    POLdir.Text := ServerPath;
    POLDirName := ServerPath;
    ConfigIni:=TIniFile.Create(TPCCfgFilePath + 'POLConfigurator.ini');
    with ConfigIni do begin
      WriteString('Startup','Users POL directory', POLDirName);
      // Save the position of the POL Configurator form.
